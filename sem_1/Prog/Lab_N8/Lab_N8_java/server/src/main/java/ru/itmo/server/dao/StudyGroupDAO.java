...
public class StudyGroupDAO implements StudyGroupDAI
{
    ...
    private static Map<FormOfEducation, Integer> formIDs = new HashMap<>();
    private static Map<Semester, Integer> semesterIDs = new HashMap<>();
    private static Map<Country, Integer> countryIDs = new HashMap<>();
    ...
    public long addGroup( long key, StudyGroup newGroup, long ownerID ) throws SQLException
    {
        ...
        {
            if (formIDs.get(newGroup.getFormOfEducation()) == null)
                throw new SQLException("Ошибка: Нет ID для формы обучения: " + newGroup.getFormOfEducation());
            if (semesterIDs.get(newGroup.getSemester()) == null)
                throw new SQLException("Ошибка: Нет ID для семестра: " + newGroup.getSemester());
            if (countryIDs.get(newGroup.getAdmin().getNationality()) == null)
                throw new SQLException("Ошибка: Нет ID для страны: " + newGroup.getAdmin().getNationality());
        ...
        {
            throw new SQLException("Не определено значение параметра: " + e.getMessage());
        }
