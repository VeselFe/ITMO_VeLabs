...
public class Server
{
    ...
    public static void handleClient( Socket clientSocket, InvokerActions invoker ) throws BDException
    {
        ...
        try
        {
            ...
            ConnectionRegistry.addClient(output);
            ...
            try( Connection dbConnection = getDB_Connection(); )
            {
                ...
                while( !clientSocket.isClosed() )
                {
                    try
                    {
                        ...
                        Future<ResponseCmdRes> responseFuture = processorPool.submit( () -> proccessor.ProcessRequest(request, dbManager, userDAO) );
                        Thread responseThread = new Thread(() -> {
                            try
                            {
                                ResponseCmdRes responseCmdRes = responseFuture.get();
                                logger.info("Запрос обработан!");
                                logger.info("<Начало запроса>");
                                logger.info("Success: " + responseCmdRes.isSuccess() + ";");
                                logger.info("Message: " + responseCmdRes.getMessage() + ";");
                                logger.info("<Конец запроса>");

                                synchronized( output )
                                {
                                    // отправляем обратно ответ
                                    byte[] responseBytes = ResponseCompiler.compileResponse(responseCmdRes);
            ...
            {
                ConnectionRegistry.removeClient(output);
            }
