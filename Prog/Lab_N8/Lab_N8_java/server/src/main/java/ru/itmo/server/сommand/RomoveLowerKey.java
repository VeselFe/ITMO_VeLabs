            ...
            else
            {
                ...
                try
                {
                    removingList.forEach(element -> {
                        try
                        {
                            collection.removeElement(args.getDBManager(), element.getKey(), args.getOwnerID());
                        }
                        catch( Exception e )
                        {
                ...
                catch( Exception e )
                {
                    return new CommandResult.Builder()
                            ...
                            .setStudyGroupList(collection.getSortedByNameCollection())
                            .buildCommandResult();
                }
                resBuilder.setMessage("Было обнаружено и удалено " + removingList.size() + " элемента(-ов): " + deletedNames);
            }
            return resBuilder.setStudyGroupList(collection.getSortedByNameCollection())
                    .buildCommandResult();
        }
        catch( Exception e )
        {
